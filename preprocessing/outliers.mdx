...
description: "Identify anomalous GHI values through exploratory scatter plots using solar zenith angle, guiding context-aware data cleaning."
---
...
plt.figure()
plt.plot(dfGeo.SZA, df.ghi, '.', ms=0.1)           # all points
plt.plot(dfGeo[dfGeo.SZA > 90].SZA,
         df.ghi[dfGeo.SZA > 90], '.r', ms=0.1)     # nighttime in red
...
Nighttime readings (SZA > 90°) are highlighted in red so they can be evaluated separately. Any GHI value that lies above the expected physical curve, or shows up in the red cluster at an unexpected magnitude, is a candidate for removal.
