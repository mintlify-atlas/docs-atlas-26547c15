---
title: "Resampling GHI Time Series from 1-Minute to Hourly Resolution"
...
counts15 = df.resample(on='datetime', rule='15min').ghi.count().values
df15 = df.resample(on='datetime', rule='15min')[['ghi']].mean().reset_index()
df15['tot'] = counts15
df15['ghi'] = np.where(df15.tot > 10, df15.ghi, np.nan)

# 60-minute resampling
counts60 = df.resample(on='datetime', rule='60min').ghi.count().values
df60 = df.resample(on='datetime', rule='60min')[['ghi']].mean().reset_index()
df60['tot'] = counts60
